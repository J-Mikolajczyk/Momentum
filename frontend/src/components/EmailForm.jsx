...
function EmailForm({ setLoggedIn, setUserInfo }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if(exists) {
      try {
        const response = await postRequest('https://'+ip+'/user/login', { email, password });
      ...
      try {
        const response = await postRequest('https://'+ip+'/user/register', { email, password, name });
        if (response.ok) {
      ...
      try {
        const response = await postRequest('https://'+ip+'/user/exists', { email });
