...
export default function BlockDashboard({blockName, setBlockName, userInfo, toggleAddBlockMenu}){
  ...
  useEffect(() => {
    if (blockData?.mostRecentDayOpen != null && blockData?.mostRecentWeekOpen != null) {
      setDisplayWeekAndDay(blockData.mostRecentWeekOpen + 1, blockData.mostRecentDayOpen + 1);

    }
  }, [blockData, setDisplayWeekAndDay]);

  return(<>
    <MessagePopup message={message} setMessage={setMessage} ignoreMethod={ignoreMethod}/>
    { blockName === null ? 
      (<div className="p-4 w-full "><div className='flex w-full items-center mb-3'>
        ...
        {userInfo?.trainingBlockNames?.length > 0 ? (
          ...
          <button onClick={() => openBlock(blockName)} key={index} className='bg-blue-50 text-blue-800 font-anton px-4 py-2 rounded-md shadow-md w-full text-left text-2xl border-blue-800 border-1 mb-1'>{blockName}</button> ))
