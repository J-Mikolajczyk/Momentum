...
export default function AddBlockPopup( {open, toggleAddBlockMenu, userInfo, setUserInfo} ) {
  const ip = import.meta.env.VITE_IP_ADDRESS;
    ...
    const handleSubmit = async (e) => {
      ...
      try {
              const response = await postRequest('https://'+ip+':8080/block/create', { blockName, userId });
              if (response.ok) {
                try {
                  const refreshResponse = await postRequest('https://'+ip+':8080/user/refresh', { email });
    ...
    return (
        <div onClick={handleOuterClick} className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div onClick={handleInnerClick} className="bg-white p-5 pr-7 rounded-xl shadow-lg w-3/4 h-1/5">
