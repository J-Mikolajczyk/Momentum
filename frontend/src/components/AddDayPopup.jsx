...
export default function AddDayPopup( {show, toggle, blockData, blockName, weekNum, update} ) {
    ...
    const addDay = async (e) => {
      e.preventDefault();
      blockData.weeks[weekNum-1].days.push(new Day(blockDayName));
      update();
      handleClose();
    }

    const [blockDayName, setBlockDayName] = useState('');
    ...
    return (
        <div onClick={handleOuterClick} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div onClick={handleInnerClick} className="bg-white p-5 pr-7 rounded-xl shadow-lg w-3/4 min-h-1/5">
            ...
            <p className='inline text-blue-900 font-anton text-2xl'>Add a day</p>
            ...
            <form>
                ...
                <button type='submit' onClick={addDay} className='mt-3 bg-white font-anton rounded-md text-blue-900 hover:bg-gray-200 transition duration-300 h-1/4 px-10 text-2xl border-blue-900 border-2'>Add</button>
